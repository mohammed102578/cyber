...
use App\Interfaces\admin\LeaderboardInterface;
...
public function leaderboard($request)
{
  try{
      ...
      ->groupBy('reporter_id')->get();
      $active="time";
      return view('content.admin.pages.general.leaderboard',compact('top_reporters','top_reporters_time','active'));
      }else{
      //top ten report now
      $top_reporters=Point::with('reporter.report')
      ->select('reporter_id',DB::raw("sum(point) as sum"),DB::raw("count(point) as count" ))
      ...
      ->groupBy('reporter_id')->get();
      $active="now";
      return view('content.admin.pages.general.leaderboard',compact('top_reporters','active'));
