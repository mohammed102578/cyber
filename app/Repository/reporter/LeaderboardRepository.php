...
use App\Interfaces\reporter\LeaderboardInterface;
      ...
      if (!empty($request->from)) {
        ...
        $top_reporters_time = Point::with('reporter.report')
         ->whereBetween('created_at', [$from, $to])
          ...
          ->groupBy('reporter_id')->get();
        $active = "time";
        return view('content.reporter.pages.leaderboard', compact('top_reporters', 'top_reporters_time', 'active'));
      } else {
        //top ten report now
        $top_reporters = Point::with('reporter.report')
          ->select('reporter_id', DB::raw("sum(point) as sum"), DB::raw("count(point) as count"))
          ...
          ->groupBy('reporter_id')->get();
        $active = "now";
        return view('content.reporter.pages.leaderboard', compact('top_reporters', 'active'));
      }
    } catch (\Exception $ex) {
